...
namespace SiegeSurvival.UI.Panels
{
    ...
    public class GameOverPanel : MonoBehaviour
    {
        ...
        private void Refresh()
        {
            ...
            if (titleText) titleText.text = $"GAME OVER — Day {s.currentDay}";

            if (causeText)
            {
                string cause = s.gameOverReason ?? "Unknown";
                causeText.text = $"Cause of Defeat: {cause}";
                causeText.color = Color.red;
            }
            ...
            if (statsText)
            {
                statsText.text =
                    ...
                    $"Siege Intensity: {s.siegeIntensity}\n" +
                    $"Laws Enacted: {s.enactedLaws.Count}\n" +
                    $"Zones Lost: {CountLostZones(s)}";
            }
        ...
        private int CountLostZones(GameState s)
        {
            int c = 0;
            foreach (var z in s.zones) if (z.isLost) c++;
            return c;
        }
